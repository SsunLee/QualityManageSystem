...
Public Class P_Project_Plan_NotUSe
...
#Region "(구현중) # 4. [변경점 내용 선택 시 변경점 내용 보여주기]"
    Private Sub trv_Type_AfterSelect(sender As Object, e As EventArgs) Handles lstChangeNote.SelectedIndexChanged
        '--------------------------------------------
        ...
        Try
            ...
            strMod = LTrim(var(1))
            strChange = lstChangeNote.SelectedItem.ToString()
...
#Region "(미사용) #3. [모델 선택 시 변경점 내용 보여주기]"
...
#Region "# 5. [Excel Donwload]"
    ...
    Private Sub Download_Click(sender As Object, e As EventArgs) Handles btnDownload.Click

        '# 지정경로 안에서 '시험기획서_Access_v1.3' 찾기 후 파일 Copy
        Dim dir As DirectoryInfo = New DirectoryInfo("\\10.169.88.40\Q-Portal\1.검증정보\최신Template\Project_기획_샘플")
        ...
        For Each dr In dir.GetFiles()
            ...
            If InStr(strFile, "시험기획_양식") And Strings.Left(strFile, 2) <> "~$" Then
                strRealFile = dr.Name
        ...
        dlg.FileName = "시험기획_양식"
        dlg.DefaultExt = ".xlsx"
        dlg.Filter = ""
        dlg.Filter = "Excel File|*.xlsx"
        dlg.Title = "Save an Excel File | lee.sunbae@lgepartner.com"
        ...
        Try
            ...
            End If
        Catch ex As Exception
            Debug.Print("닫혔다 --> Exception으로" & ex.Message)
        End Try

    End Sub

    Public Sub Sample_saveExcelFile(ByVal strFileName As String, ByVal FileName As String)
...
#Region "# 5. [Excel File Upload] "

#Region "★ Form 띄우기"
    Private Sub btnUpload_Click(sender As Object, e As EventArgs) Handles btnUpload.Click
        'Attact_File
        Dim Attact_File As New P_Project_Plan_Sub_Upload_NotUse
        With Attact_File
            .Owner = Me     '# had a Ownership
            .ShowDialog()     '# append a form like dialog 
        End With
