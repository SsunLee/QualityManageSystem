...
Public Class M_Model_Plan_Upload
    'Public DataGridView1 As New DataGridView   '# DataGridView 생성
#Region "★[처음 로드될 때]★"
...
#Region "★[엑셀 다운로드 버튼 클릭]★"
    Private Sub Pic_Excel_down_Click(sender As Object, e As EventArgs) Handles Pic_Excel_down.Click
        ...
        Dim strPath As String = DownExcel("\\10.169.88.40\Q-Portal\2.검증현황\시험기획", "MODEL_시험기획.xlsx")
...
#Region "★ 엑셀 to DataGridView 함수"
    Private Function Excel_File_to_DataGridView(ByRef path As String) As Boolean
        ...
        myCmd = New System.Data.OleDb.OleDbDataAdapter("Select * FROM [Sheet1$]", vConn)
...
#Region "★ DataGridView to DB 함수"
    Private Function Database_Update(ByRef dt As DataTable) As String
        ...
        Dim reader As OleDbDataReader   '// datareader 선언

        Dim DA As System.Data.OleDb.OleDbDataAdapter = New System.Data.OleDb.OleDbDataAdapter
        Dim vConn = New System.Data.OleDb.OleDbConnection()   ' MainForm의 Connection String으로 DB Connect
        vConn.ConnectionString = Con.strCon
        ...
        Try
            ...
            For i As Integer = 0 To dt.Rows.Count - 1
                ...
                sql = "SELECT ID "
                sql = sql & "FROM [시험기획_DB_VER_TWO] "
                sql = sql & "WHERE ID > 0  " & vSQL2_Major & vSQL_Pro & vSQL_Mod & vSQL_Step & vComp ' & vSQL_Change_Contents '&' vSQL_TestType_Contents
                Debug.Print(sql)
                ...
                Dim test_num As Integer = 0

                While reader.Read
                    test_num += 1
                    Debug.Print(reader(0))  '// ID 값을 기준으로 덮어쓰기 및 INSERT 여부 Check 하기 때문
                End While

                Debug.Print(test_num.ToString)
                ' = cmd.ExecuteReader()
                ...
                Else '◀◀ 기존 데이터와 덮어쓰기 해야 한다면
                    ...
                    sql = sql & " WHERE ID = " & reader(1) 'result.Rows(0)(0).ToString()
                    Debug.Print(sql)
